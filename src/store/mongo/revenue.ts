...
import { ESortOrder, IRevenue, IRevenueFilter } from 'interface';
import { Revenue } from 'model';
import { BaseStore } from './base';

export class MongoRevenue extends BaseStore<IRevenue> {
  constructor(db: Db) {
    super(db, 'revenues');
  }
  ...
  private getQuery(filters: IRevenueFilter) {
  ...
  async getPaginate(filters: IRevenueFilter) {
    ...
    const result = await this.collection
      .aggregate<{ data: IRevenue[]; pageInfo: Array<{ count: number }> }>([
        {
          $match: condition,
        },
        {
  ...
  async getList(filters: IRevenueFilter) {
    ...
    return this.collection.find<IRevenue>(condition, { projection: this.getProject() }).toArray();
  }

  async getOne(filters: IRevenueFilter) {
    const { condition } = this.getQuery(filters);
    return this.collection.findOne<IRevenue>(condition, { projection: this.getProject() });
  }

  async createOne(data: Revenue, session?: ClientSession) {
    data.preSave();
  ...
  async updateOne(id: string, data: Revenue, session?: ClientSession) {
    data.preUpdate();
