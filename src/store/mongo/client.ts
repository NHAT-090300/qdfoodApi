...
import { ESortOrder, IClient, IClientFilter } from 'interface';
import { Client } from 'model';
import { BaseStore } from './base';

export class MongoClients extends BaseStore<IClient> {
  constructor(db: Db) {
    super(db, 'clients');
  }
  ...
  private getQuery(filters: IClientFilter) {
  ...
  async getPaginate(filters: IClientFilter) {
    ...
    const result = await this.collection
      .aggregate<{ data: IClient[]; pageInfo: Array<{ count: number }> }>([
        {
          $match: condition,
        },
        {
  ...
  async getList(filters: IClientFilter) {
    ...
    return this.collection.find<IClient>(condition, { projection: this.getProject() }).toArray();
  }

  async getOne(filters: IClientFilter) {
    const { condition } = this.getQuery(filters);
    return this.collection.findOne<IClient>(condition, { projection: this.getProject() });
  }

  async createOne(data: Client, session?: ClientSession) {
    data.preSave();
  ...
  async updateOne(id: string, data: Client, session?: ClientSession) {
    data.preUpdate();
